...
import {UNI_COLORS, MAJOR_CATEGORIES, MAJOR_COLORS, EVENT_CODE} from './constants.js';
...
class P5Background extends React.Component {
	...
	Sketch = (p5) => {
		...
		p5.setup = () => {
			p5.createCanvas(	
						p5.windowWidth - windowMargin,
						p5.windowHeight - windowMargin
						);
		...
		p5.windowResized = () => {
			p5.resizeCanvas(p5.windowWidth - windowMargin, p5.windowHeight - windowMargin);
			for (let i = 0; i < MAJOR_CATEGORIES.length; i++){
	...
	componentDidMount() {
		this.myP5 = new p5(this.Sketch, this.myRef);
	}
